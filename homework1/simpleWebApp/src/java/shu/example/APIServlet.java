...
public void service(@NotNull ServletRequest servletRequest,
	...
	if (!tokenValid(request)) {
		response.setHeader("WWW-Authenticate",
				"X-Token realm=\"Foglight Usage\"");
		Handler.sendError(response, HttpServletResponse.SC_UNAUTHORIZED,
				"Not authorized");
		return;
	}
...
private static String getToken(@NotNull HttpServletRequest request) {
	...
	if (auth != null) {
		return auth.startsWith("X-Token ") ? auth.substring(8) : null;
	}

	return request.getParameter(".token");
}

private static boolean tokenValid(@NotNull HttpServletRequest request) {
	//todo
//	String token = getToken(request);
//	return token != null && TOKENS.computeIfPresent(token,
//			(k, v) -> v.check(request)) != null;
	return true;
}
